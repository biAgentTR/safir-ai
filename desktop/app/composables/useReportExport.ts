 ...
 *   - JSON : the ŞARTNAME-required shape (`summary`/`onset_timestamp`/
 *            `safe_timestamps`/`incident_timestamps`/`events`/`risk`/
 *            `risk_score`/`risk_status`/`actions`) — see `buildSartnameJson()`,
 *            a line-for-line mirror of the backend's authoritative
 *            `SafirReport.to_sartname_json()` (src/schemas/report.py). This is
 *            NOT the full internal report — the competition JSON deliverable
 *            is exactly this reduced, spec-mandated shape, built client-side
 *            from the already-loaded report (no extra request).
...
/**
 * Şartnamedeki mock örnekle birebir aynı şekle indirger — bkz.
 * `SafirReport.to_sartname_json()` (src/schemas/report.py), buranın tek
 * gerçek kaynağı (source of truth). Bu iki uygulama birbirinden BAĞIMSIZ
 * ÇALIŞIR (biri backend/Python, diğeri istemci/TypeScript) — alan adları
 * veya davranış değiştirilecekse HER İKİSİ de güncellenmelidir.
 */
