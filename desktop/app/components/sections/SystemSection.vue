...
// No longer a routed page (moved into the single-scroll hub, pages/index.vue)
// so there is no route middleware to gate it — the same admin-only guard
// that used to live in middleware/admin-auth.ts is enforced inline below
// (both the template's v-if and skipping the data loads in onMounted).
...
<template>
  <div id="sistem" class="scroll-mt-16 max-w-6xl mx-auto px-6 py-8">
    <!-- demo admin gate: this section used to be a route guarded by
         middleware/admin-auth.ts; as a scroll section it guards itself -->
    <div v-if="!auth.isAuthenticated" class="glass-panel rounded-lg p-8 text-center max-w-md mx-auto">
      <div class="text-2xl mb-2" aria-hidden="true">🔒</div>
      <h2 class="text-lg font-bold text-slate-100">Sistem Verileri yönetici erişimi gerektirir</h2>
      <p class="mt-1 text-sm text-slate-500">İşleme hattı, model ve sistem kayıtlarını görmek için oturum açın.</p>
