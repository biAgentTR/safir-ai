...
<template>
  <div id="ana-sayfa" class="scroll-mt-16 relative">
    <!-- Dikey nefes: bolumler arasi mesafe bilerek buyutuldu (py-16/20 ->
         py-24/32) - hero, hemen altindaki "Direct Analiz" basligiyla ayni
         ekranda sikismasin, her blok kendi alanina sahip olsun. -->
    <div class="relative max-w-4xl mx-auto px-6 py-24 sm:py-32 text-center">
      ...
      <h1 class="text-5xl sm:text-6xl lg:text-7xl font-bold tracking-tight text-slate-100 leading-[1.05] relative z-10">
      ...
      <!-- Kartlar buyutuldu (p-4.5 -> p-6, rounded-xl -> rounded-2xl) ve
           HIZALANDI: satir `items-start` yerine `items-center` - ikon, metin
           ve ok artik ayni dikey eksende. Onceden ok yukari yapisiyordu ve
           iki satirlik aciklamalarda kart icerigi kaymis gorunuyordu. -->
      <div class="mt-14 grid grid-cols-1 sm:grid-cols-2 gap-4 text-left relative z-10">
        <button
          v-for="l in links"
          :key="l.id"
          type="button"
          class="relative overflow-hidden group rounded-2xl p-6 flex items-center justify-between gap-4 bg-surface-1/90 backdrop-blur-md border border-edge/80 hover:border-accent/60 shadow-lg hover:shadow-[0_12px_28px_-6px_rgba(20,184,166,0.18)] hover:-translate-y-0.5 transition-all duration-300 cursor-pointer text-left"
          @click="onCardClick(l)"
        >
          ...
          <div class="flex items-center gap-4 min-w-0">
            <span class="w-12 h-12 rounded-xl bg-accent/15 border border-accent/30 text-accent flex items-center justify-center text-xl shrink-0 shadow-[0_0_12px_rgba(20,184,166,0.2)] group-hover:scale-105 group-hover:bg-accent/25 transition-all duration-200" aria-hidden="true">{{ l.glyph }}</span>
            <span class="min-w-0">
              <span class="block text-base font-semibold text-slate-100 group-hover:text-accent transition-colors">{{ l.label }}</span>
              <span class="block mt-1 text-sm text-slate-400 group-hover:text-slate-300 transition-colors leading-relaxed">{{ l.blurb }}</span>
            </span>
          </div>

          <span class="text-base text-slate-500 group-hover:text-accent group-hover:translate-x-1 transition-all duration-200 shrink-0">→</span>
        </button>
      </div>
    </div>
  </div>
