...
const contributionRows = computed(() => {
  ...
  return Object.keys(CONTRIBUTION_LABELS)
    .filter((key) => key in contributions && contributions[key] != null)
    .map((key) => ({ key, label: CONTRIBUTION_LABELS[key], value: contributions[key] as number }))
})

const hasMetrics = computed(
  () => featureRows.value.length > 0 || contributionRows.value.length > 0 || !!store.report?.contributing_rule_ids?.length,
)
</script>
  ...
  <div v-if="store.report" class="space-y-2.5">
    ...
    <div v-if="hasMetrics" class="card p-4">
      ...
      >
        ...
        <span class="text-xs text-slate-400 font-medium">{{ metricsOpen ? '▾ Gizle' : '▸ Göster' }}</span>
      </button>
      ...
      <div v-if="metricsOpen" class="mt-3 space-y-4">
        <div v-if="featureRows.length" class="space-y-2">
          ...
          <div v-for="row in featureRows" :key="row.key" class="flex items-center gap-3 text-sm">
            <div class="w-44 shrink-0 text-slate-300 text-xs">{{ row.label }}</div>
        ...
        <div v-if="contributionRows.length" class="space-y-1.5">
          ...
          <div class="grid grid-cols-2 sm:grid-cols-4 gap-2 text-sm">
            <div v-for="row in contributionRows" :key="row.key" class="rounded-md border border-edge bg-surface-2 px-2.5 py-1.5">
