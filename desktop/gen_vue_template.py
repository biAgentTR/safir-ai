    ...
    <div class=\"relative z-10 w-full max-w-[1320px] mx-auto px-6 lg:px-10 pt-8\">
      ...
      <div v-else-if=\"(state === 'completed' || state === 'empty') && report\" class=\"flex flex-col\">
        ...
        <div class=\"flex flex-col md:flex-row md:items-start justify-between mb-8 gap-4\">
          ...
          <div>
            <button @click=\"scrollToDetails\" class=\"px-5 py-2.5 rounded-lg border border-[var(--color-border)] text-[var(--color-text-secondary)] hover:text-white hover:bg-[var(--color-surface-hover)] transition-colors text-xs font-medium flex items-center gap-2\">
        ...
        <div class=\"grid grid-cols-1 lg:grid-cols-[minmax(0,2fr)_minmax(320px,0.95fr)] gap-4 mb-8\">
          <div class=\"flex flex-col min-w-0\">
            <div class=\"aspect-video bg-[#05090c] rounded-[16px] border border-[var(--color-border)] overflow-hidden\">
              <ReportVideoViewer ref=\"videoViewer\" :videoPath=\"report.video_source\" />
            </div>
            <div class=\"grid grid-cols-3 gap-px bg-[var(--color-border)] border border-[var(--color-border)] mt-4 rounded-[12px] overflow-hidden\">
        ...
        <div v-if=\"state === 'completed'\" class=\"flex flex-col sm:flex-row sm:items-center justify-between bg-[var(--color-surface)] border border-[var(--color-border)] rounded-[16px] p-5 mb-16 gap-4\">
          <div class=\"flex items-center gap-4\">
            ...
            <div>
              <div class=\"text-[13px] font-bold text-[var(--color-text)]\">Olay analiz raporu hazýr</div>
              <div class=\"text-[11px] text-[var(--color-text-secondary)]\">Zaman çizelgesi, kritik kareler ve operatör aksiyonlarýný içerir.</div>
            </div>
          </div>
          <div class=\"flex items-center gap-3\">
            <ReportDownloadButton :job-id=\"jobId\" :video-name=\"report ? getBasename(report.video_source) : ''\" />
            <button @click=\"scrollToDetails\" class=\"px-5 py-2.5 rounded-lg border border-[var(--color-border)] text-[var(--color-text-secondary)] hover:text-white hover:bg-[var(--color-surface-hover)] transition-colors text-xs font-medium flex items-center gap-2 shrink-0\">
              Raporu görüntüle
        ...
        <div id=\"report-details\" class=\"space-y-6 pt-4\">
          <div class=\"grid grid-cols-1\">
            <ReportSummary
              :naturalLanguageSummary=\"report.natural_language_summary\"
              :summary=\"report.summary\"
            />
          </div>
          ...
          <div class=\"grid grid-cols-1 lg:grid-cols-2 gap-6 min-h-[400px]\">
            ...
            />
            <ReportRagSources
              :sources=\"report.semantic_rag_sources\"
            />
          </div>
        </div>
