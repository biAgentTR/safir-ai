...
"""
text = text.replace("from typing import Any, Callable, Dict, List, Optional, Tuple", "from typing import Any, Callable, Dict, List, Optional, Tuple\n" + import_stmt)

# 2. Add overlap_sec to analyze_video
...
text = text.replace(old_chunker, new_chunker)

# 3. Replace _analyze_video_chunks entirely
import re
match = re.search(r"    def _analyze_video_chunks.*?    def _send_single_video", text, re.DOTALL)
if match:
    old_method = match.group(0)[:-26] # remove the next method signature
    new_method = """    def _analyze_video_chunks(
        ...
        for chunk in chunks:
            ...
            try:
                ...
                if not res:
                    ...
                    if not res.analysis_id and getattr(chunk, 'context', None): res.analysis_id = chunk.context.analysis_id
                    if not res.video_id and getattr(chunk, 'context', None): res.video_id = chunk.context.video_id
