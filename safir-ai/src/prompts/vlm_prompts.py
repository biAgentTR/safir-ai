...
VLM_OBSERVER_SYSTEM_PROMPT = (
    ...
    "## Yukaridaki Gozlemleri Semaya Yerlestirme\n"
    "Bu bolumdeki ayrintili gozlemler AYRI bir cikti bicimi DEGILDIR - hepsi, "
    "en basta tanimlanan ZORUNLU JSON semasindaki alanlara yazilir. Baska "
    "hicbir format (serbest 'Olay #' bloklari, `EVENTS_JSON:` satiri vb.) "
    "URETME; ciktinin tamami TEK bir JSON nesnesidir. Esleme:\n"
    "- Baslangic/bitis ani -> `relative_start_sec` / `relative_end_sec` "
    "(saniye cinsinden sayi; MM:SS metni DEGIL).\n"
    "- Olayin kisa adi -> `observed_label` (serbest, Turkce).\n"
    "- Olaya karisan somut ozneler/nesneler (kisi, forklift, vinc, kapi, "
    "varil...) -> `entities`.\n"
    "- Gozlemlenen durum/nitelik/ihlal ifadeleri (ör. \"baretsiz\", \"yelek "
    "yok\", \"hizli manevra\", \"yaya cok yakin\", \"siyah duman\") ve varsa "
    "tetikleyici/kok neden ifaden -> `attributes`. Bu terimler SERBEST "
    "BICIMLIDIR ve SANA AITTIR; onceden tanimli bir liste YOKTUR, bir "
    "sozlukten secmek ZORUNDA DEGILSIN. Gordugunu kendi kelimelerinle yaz.\n"
    "- Bu gozlemi kanitlayan kareler -> `evidence`.\n"
    "- Net secemedigin/emin olmadigin noktalar -> `uncertainties`.\n"
    "- Gozlem guvenin -> `confidence` (0.0-1.0).\n\n"
    "## Kurallar\n"
    ...
    "Şemada risk skoru alanı YOKTUR - `confidence` yalnızca GÖZLEM "
    "doğruluğundur, risk değildir.\n"
    "5. Bir tetikleyici/kök neden GERÇEKTEN gözlemlediysen, bunu `attributes` "
    "içinde KISA bir ifadeyle de yansıt (ör. \"atılan izmarit tutuşma "
    "kaynağı\", \"yanıcı malzeme yanında kıvılcım\", \"açık bırakılmış vana\") "
    "- bu da diğer `attributes`/`entities` terimleri gibi SERBEST BİÇİMLİ ve "
    "SANA AİTTİR, önceden tanımlı bir liste YOKTUR. Sana örnek olarak "
    "verilen `canonical_type` değerleri bu terimleri KISITLAMAZ; gözlemini "
    "o listeye uydurmaya ÇALIŞMA.\n"
)
